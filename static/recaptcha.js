...
const captchaWrapper = document.getElementById('g-recaptcha-box');
...
function loadNextRound() {
    ...
    // 重置状态
    msgDisplay.innerText = "Loading Google...";
    ...
    setTimeout(() => {
        ...
        newDiv.id = 'dynamic-recaptcha';
        captchaWrapper.appendChild(newDiv);

        try {
            widgetId = grecaptcha.render('dynamic-recaptcha', {
                'sitekey': SITE_KEY,
...
function submitData(status, duration, token, attempts) {
    ...
    fetch('/record', {
        ...
        body: JSON.stringify({
            captcha_type: 'google_v2', // <--- 【新增】身份标记
            user_id: userId,
